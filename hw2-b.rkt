...
;; time the sorting of some algorithm until max_duration_ms is reached
;; returns the number of elements in the list when the duration was exceeded
(define (time-sort sortm max-duration-ms [list-builder randomlist] [current-size 100] [increment 100])
  (let* ([random-list (list-builder current-size max-int-value)]
         [start-time (current-inexact-milliseconds)]
         [sorted-list (sortm random-list)]
         [sort-time (- (current-inexact-milliseconds) start-time)])
    (if (< max-duration-ms sort-time)
        current-size
        (time-sort sortm max-duration-ms list-builder (+ current-size increment) increment))))


;; But what we really need is a CSV that produces list size, merge sort ms, insertion sort ms
(define (time-both-sorts max-duration-ms [results-list '()] [current-size 100] [increment 100])
  (define merge-sort-ms
    (let* ([random-list (range 1 current-size)]
           [start-time (current-inexact-milliseconds)]
